...
let test_eq_formula_different_structure () =
  ...
  check bool "Imp(p_x, p_y) != Bot" false (eq_formula f1 Bot |> not)

let test_eq_formula_alpha_equiv () =
  (* With the current naive eq_formula, these might fail if eq_formula
     compares raw strings and doesn't do alpha-renaming. *)
